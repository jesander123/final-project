    ...
    def draw(self):
        ...
        for s in self.shapes:
            ...
            if s["type"] == "circle":
                ...
                points = []
                for x_global in range(self.width): # for every pixel along the width of the screen
                    y_global = int(s["y"] + amplitude * np.sin(2 * np.pi * x_global / wave_length)) # apply sine wave
                    points.append((x_global, y_global))

                pygame.draw.lines(
                    self.screen, # suface is entire screen
                    color,
                    False, # do no close the shape, only show a line
                    points, 
                    thickness)
                continue #skip blit as it is directly drawn on the surface of the screen

            self.screen.blit(surf, (s["x"] - cx, s["y"] - cy))
            # blit copied one surface onto another, replace surf with the window screen
